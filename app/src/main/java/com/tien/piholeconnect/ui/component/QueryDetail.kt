...
fun QueryDetail(
    ...
    val (icon, tint) = when (query.answerType.category) {
        ...
        AnswerCategory.ALLOW -> Pair(
            Icons.Default.GppGood, MaterialTheme.colorScheme.success
        )

        AnswerCategory.CACHE -> Pair(
            Icons.Default.Cached, MaterialTheme.colorScheme.info
        )
        ...
        Column(Modifier.padding(top = 8.dp)) {
            ListItem(
                ...
                headlineContent = {
                    SelectionContainer {
                        Text(
                            query.answerType.name, fontWeight = FontWeight.Bold, color = tint
                        )
                    }
                })
            ListItem(leadingContent = { Icon(Icons.Default.Schedule, contentDescription = null) },
                overlineContent = { Text(stringResource(R.string.query_detail_timestamp)) },
                headlineContent = { SelectionContainer { Text(dateFormat.format(query.timestamp * 1000L)) } })
            ListItem(leadingContent = { Icon(Icons.Default.Domain, contentDescription = null) },
            ...
            ListItem(leadingContent = { Icon(Icons.Default.Devices, contentDescription = null) },
                overlineContent = { Text(stringResource(R.string.query_detail_client)) },
                headlineContent = { SelectionContainer { Text(query.client) } })
            ListItem(leadingContent = { Icon(Icons.Default.Dns, contentDescription = null) },
            ...
            ListItem(leadingContent = {
                Icon(
                    Icons.Default.HourglassBottom, contentDescription = null
                )
            },
        ...
        Column(
            ...
            Box(contentAlignment = Alignment.Center) {
                TextButton(
                    ...
                    }, onClick = onWhitelistClick, enabled = !loading
            ...
            Box(contentAlignment = Alignment.Center) {
                TextButton(
                    ...
                    }, onClick = onBlacklistClick, enabled = !loading
...
fun QueryDetailPreview() {
    PiHoleConnectTheme {
        QueryDetail(PiHoleLog(
            ...
            responseTime = 450
        ), onWhitelistClick = {}, onBlacklistClick = {}, onDismissRequest = {})
    }
}
